...
2. Block the stop when the fixer rewrote a file this session's own last commit
   already included. That commit no longer carries the reformat, and the turn is
   ending, so the diff would sit stranded until something prompts a fresh
   `git status`. The block states what happened and stops there - what to do
   about it is Claude's call, not the hook's.
...
def run_fixer(root):
    """Run the repo's configured fix command; it and the paths it rewrote.
...
def stranded(root, rewritten, transcript):
    """Fixer-rewritten paths this session's own last commit already included.

    Reformatting a file the session hasn't committed leaves the change in the
    working tree, where the next commit picks it up - nothing is stranded, so
    the stop isn't worth interrupting. Reformatting one the session did commit
    leaves that commit behind.
    """
    if not rewritten:
        return []
    ...
    return sorted(rewritten & committed)
...
def main():
    ...
    command, rewritten = run_fixer(root)
    result = {}
    if rewritten:
        result["systemMessage"] = f"Applied fixes ({command})"

    files = stranded(root, rewritten, payload.get("transcript_path"))
    if files:
