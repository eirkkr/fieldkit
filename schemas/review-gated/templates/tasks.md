...
- [ ] 3.1 Walk the full diff for the conventions CI cannot check. Done
      ...
      findings, and it changes nothing on its own.
...
- [ ] 3.7 FINAL REVIEW - present the change as a whole and iterate with the
      reviewer until they are satisfied. The note opens with two diffs: the
      work since the last approval, then the whole change, each with its base
      commit named. Stop here. Only the reviewer closes this box.
