...
pytest config must live under `[tool.pytest.ini_options]` in `pyproject.toml`.
The `[tool.pytest]` table is silently ignored - no error is raised, but none of
the settings take effect. Always verify the table header when config appears to
have no effect.
